...
Vary tank size:
    tank1: (w,h,d) = (24,12,10)
    tank2: (w,h,d) = (30,12,12)
...
dimension = '3d'    # 2d/3d
# columns are width, height, depth, rows are indiv tanks
n_tanks = 2
tank_size = np.array([[24.,12.,6.],[30.,12.,12.]])
swarm_size = np.array([20,12,10])# number of agents
...
for iter in range(n_iter):
    for tank in range(n_tanks):
        field_width, field_height, field_depth = tank_size[tank,]
        for n in swarm_size:
            max_steps = np.round(np.random.uniform(low = time_sample_low,high = time_sample_high))
            r_a_input = r_o + r_a
            group_dir,group_rho = swarm_wrapper(n,max_steps,field_width,field_height,field_depth,r_o,r_a,dimension)
...
#output_df.to_csv('couzin_simfield_example2.csv',index = False)
